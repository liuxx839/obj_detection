...
# 文字信息：尽可能提取图片里的文字信息
# 人物信息（若包含人）：
・人头数：X 人（精确计数）。请仅提供一个准确结果。
  - 计数容错标准：
    * 若图片中总人数 ≤ 8人，允许误差±0人
    * 若图片中总人数 >8且≤15人，允许误差±1人
    * 若图片中总人数 >15且≤20人，允许误差±2人
・性别比例：男性 X 人，女性 Y 人，性别不明 Z 人（无人物则填 0）。
・种族比例：黑人，白人，亚洲人等
・年龄段：按儿童（＜12 岁）、青年（12-35 岁）、中年（36-60 岁）、老年（＞60 岁）分类，标注各年龄段人数（如 "青年 2 人，中年 1 人"）。
・着装情况：是否有人穿着医疗专业人员制服（白大褂、护士服、手术服）vs. 普通便服
...
# 验证项
以下是针对特定场景的验证，需基于图片可见内容判断，而非主观推测：

## 图片风格
是否为真实照片，还是某种风格图像？是否为视频截图或翻拍，还是自拍？

## 空场景识别
图片是否为无人、无明显物体的空场景（如空白墙面、纯色背景）？
识别结果：是/否/无法判断（若有物体或人物，标注 "非空场景"）。

## 会议相关性
场景是否与会议无直接关联（如纯自然风景、单独静物、娱乐场所等）？
...
## 会议场所合规性
场所类型：正常会议场所（如会议室）/非正常会议场所（具体描述，如 "KTV 包厢""公园草坪""餐厅大堂""野外露营"等）/无法判断。
...
## 非合规物件识别
是否出现与会议无关的非合规物件（如酒瓶、麻将桌、酒杯，游戏机等）？
...
0. Sanofi相关性：检查是否有Sanofi的logo或明确与Sanofi相关的因素。注意：对于明确的Sanofi logo本身，不建议直接判断为违规。

1. 主体内容合规性：提取AI分析结果中的主体内容描述，判断描述的场景、活动和元素是否符合医药行业合规要求。

2. 场景合规性：判断是否为正式会议场所（根据主体内容信息判断）
   - 室外场景不被允许
   - 非正常会议场所（如餐厅大堂、KTV包厢、野外露营等）不被允许

3. 人员情况：
   - 人数必须≥1人（若人数为0，不被允许）
   - 人数准确性：综合比较AI分析和CV检测的人数结果，给出合理的人数
   - 检查着装是否得体（特别注意是否有医疗专业人员制服）
   - 人员行为是否专业恰当

4. 物品合规性：是否有违规物品（如酒精饮品、麻将桌、游戏机等）

5. 图片真实性：检查是否为真实照片（仅真实照片被允许）

6. 图片来源格式：检查是否为截图、翻拍或自拍（均不被允许）

7. PPT内容：若图片包含PPT幻灯片，评估其内容是否适当

8. 文字信息合规性：总结分析结果中提取的图片文字信息（如有），判断内容是否合适

9. 图片尺寸：是否存在拉伸或压缩，是否为非标准照片尺寸
   - 若非标准尺寸，仅提醒可能被裁剪，不建议直接判断为违规
   - 常见标准尺寸参考：
     * 1.0: "1:1 (正方形)"
     * 1.33: "4:3 (常见屏幕)"
     * 1.5: "3:2 (传统相机)"
...
10. 整体评估：基于以上审核项目，给出最终建议（是否建议通过审核）
