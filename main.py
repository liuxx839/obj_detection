...
with st.sidebar:
    ...
    model_option = st.selectbox(
        "选择模型",
        ["YOLOv8x (最高精度，较慢)", "YOLOv8l (高精度)", "YOLOv8m (中等精度)", "YOLOv8s (较快)", "YOLOv8n (最快，精度较低)"],
        index=0
    )
...
def detect_face(image, face_detector, conf_threshold):
   ...
   # 检测人脸，使用conf_threshold作为minNeighbors的基础
    # 将conf_threshold（0-1范围）转换为适合minNeighbors的值（通常3-6）
        ...
        if current_upload_id != st.session_state.last_upload_id or not st.session_state.processed:
            ...
            with st.spinner(f"正在使用 {model_option} 进行物体检测..."):
                ...
                if detect_faces:
                    ...
                    # 在原始图像上检测人脸位置
                    img_gray = cv2.cvtColor(original_image_for_face, cv2.COLOR_RGB2GRAY)
                    min_neighbors = max(3, int(face_confidence * 10))
                    faces = face_detector.detectMultiScale(
                        img_gray,
                        scaleFactor=1.1,
                        minNeighbors=min_neighbors,
