...
# 添加人脸聚类函数
def face_clustering(input_folder, output_folder, detection_method="hog", tolerance=0.6, min_cluster_size=3):
    """
    ...
    for (i, image_path) in enumerate(image_paths):
        ...
        try:
            ...
            # There could be multiple faces in an image
    ...
    print(f"[INFO] Detected {len(known_encodings)} faces in total")
...
def process_face_for_advanced_search(detected_faces, tolerance=0.6):
    ...
    for face_idx, face in enumerate(detected_faces):
        try:
            ...
            # 保存为临时文件，与原始代码保持一致的处理方式
            temp_face_path = os.path.join(current_dir, f"temp_face_{face_idx}_{random.randint(0, 10000)}.jpg")
            cv2.imwrite(temp_face_path, cv2.cvtColor(rgb_face, cv2.COLOR_RGB2BGR))
            
            # 使用与离线工具相同的方法进行人脸检测和编码
            image = cv2.imread(temp_face_path)
            rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            
            # 使用face_recognition进行人脸检测和编码
                ...
                for img_file in sample_images:
                    ...
                    try:
                        ...
                        if face_locations:
                            ...
                            cluster_encodings.extend(encodings)
                    except Exception as e:
                        print(f"处理图片 {img_path} 时出错: {e}")
                
                if not cluster_encodings:
                    continue
                
                # 计算当前人脸与聚类中所有人脸的相似度
                similarities = face_recognition.face_distance(cluster_encodings, current_face_encoding)
                
                # 取最高相似度（最小距离）
                if len(similarities) > 0:
                    similarity = 1 - min(similarities)  # 转换距离为相似度
                    
                    # 如果相似度高于阈值且高于当前最高相似度
                    if similarity >= (1 - tolerance) and similarity > best_similarity:
                        # 查找对应的蒙太奇图
...
def summarize_batch_faces(similarity_threshold=0.5):
    ...
    try:
        ...
        face_clustering(temp_input_dir, temp_output_dir, 
                        ...
                        tolerance=.3,
                        min_cluster_size=2)  # 设置min_cluster_size为1以包含所有人脸
